...
int main()
{
    ...
    string alphabet, line, temp, word;
    string inputName;
    ...
    while (1)
    {
        ...
        if (in.is_open())
        {
            ...
            while (getline(in,line))
            {
                ...
                numOfLines++;
                //if the lines first character is a carriage return OR line is equal to new line character
                if(line[0] == (char)(13) || line  ==  ("\n") )
                    prevLineIsEmpty = true; //the previous line is empty

                //IF THE PREV-LINE IS EMPTY, AND THE CURRENT IS NOT->1 PARAG.
                if (prevLineIsEmpty && line[0] != (char)(13))
                {
                    numOfPars++;
            ...
            //ADD INFO OF 10 MOST USED WORD FROM MAP TO VECTO THEN CLEAN UP.
            int counter = 0;
                     ...
                     itt != it->second.end(); ++itt)
                {
                    ...
                    for (; i < itt->second.size() ; ++i)
                    {
                       print += to_string(itt->second[i]) + " ";
                        if ((i+1)%10 == 0)  // 10 ITEMS PER LINE.
                        {
                            toPrint.push_back(w + "-P#" + to_string(itt->first) + "-L#: " + print);
                            print.clear();
                        }
                    }
                    if (i < 10)
